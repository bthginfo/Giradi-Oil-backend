...
exports.default = shipmentCreatedHandler;
const resend_1 = require("resend");
const resend = new resend_1.Resend(process.env.RESEND_API_KEY);
async function shipmentCreatedHandler({ event, container, }) {
    console.log("🚚 [Subscriber] order.fulfillment_created fired – data:", JSON.stringify(event.data));
    // This event fires when a shipment/fulfillment is created via the admin.
    // The fulfillment-created.ts subscriber handles the fulfillment.created event
    // which may not always fire for admin-initiated shipments.
    // This subscriber catches the order-level event as a fallback.
    try {
        ...
        const { data: [order] } = await query.graph({
            ...
            fields: [
                ...
                "fulfillments.id", "fulfillments.labels.*",
            ],
        });
        if (!order) {
        ...
        const label = latestFulfillment?.labels?.[0];
    ...
    <div style="max-width:480px;margin:0 auto;background:#1a1a14;color:#FAF8F3;font-family:Georgia,serif;padding:32px;border-radius:12px;">
      ...
      <p style="text-align:center;color:#666;font-size:13px;margin-top:24px;">The Girardi Oil / 1000 Horia</p>
        ...
        console.log(`✅ [Subscriber] Shipment email sent to: ${order.email}`);
    }
    catch (err) {
        console.error("❌ [Subscriber] order.fulfillment_created error:", err.message);
    }
}
exports.config = {
